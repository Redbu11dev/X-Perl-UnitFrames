...
local function XPerl_Player_UpdateManaType()
	XPerl_SetManaBarType("player", XPerl_Player_StatsFrame_ManaBar)
...
local function XPerl_Player_UpdatePVP()
	...
	if (UnitAffectingCombat("player")) then
		XPerl_Player_NameFrame_NameBarText:SetTextColor(1,0,0)
	elseif (XPerlConfig.ClassColouredNames == 0 and UnitIsPVP("player")) then
		XPerl_Player_NameFrame_NameBarText:SetTextColor(0,1,0)
	else
		XPerl_ColourFriendlyUnit(XPerl_Player_NameFrame_NameBarText, "player")
	end

	if (XPerlConfig.ShowPlayerPVP == 1 and UnitIsPVP("player")) then
...
local function XPerl_Player_UpdateHealth()
	...
	XPerl_SetHealthBar(XPerl_Player_StatsFrame_HealthBar, playerhealth, playerhealthmax, XPerlConfig.HealerModePlayer, XPerlConfig.HealerModePlayerType)
...
function PlayerStatus_OnUpdate(val, max)
	if (XPerlConfig.FullScreenStatus == 1) then
		local testLow = XPerlConfig.FullScreenStatusWarn / 100
		local testHigh = XPerlConfig.FullScreenStatusOK / 100

		if (val and max) then
			local test = val / max

			if ( test <= testLow and not LowHealthFrame.flashing and not UnitIsDeadOrGhost("player")) then
				UIFrameFlash(LowHealthFrame, 0.5, 0.5, 100)
				LowHealthFrame.flashing = 1
			elseif ( (test >= testHigh and LowHealthFrame.flashing) or UnitIsDeadOrGhost("player") ) then
				if (LowHealthFrame.flashMode == "IN") then
					LowHealthFrame.flashDuration = (LowHealthFrame.flashTimer or 0) + 0
				else
					LowHealthFrame.flashDuration = (LowHealthFrame.flashTimer or 0) + 1
				end
			...
			elseif ( not UIParent.isOutOfControl and OutOfControlFrame.flashing ) then
				if (OutOfControlFrame.flashMode == "IN") then
					OutOfControlFrame.flashDuration = (OutOfControlFrame.flashTimer or 0) + 0
				else
					OutOfControlFrame.flashDuration = (OutOfControlFrame.flashTimer or 0) + 1
				end
...
-- Event Handler --
-------------------
function XPerl_Player_OnEvent()		-- TBC - function XPerl_Player_OnEvent(self, event)
...
function XPerl_Player_GetGap()
	if (XPerl_Target) then
		local playerLeft = XPerl_Player_StatsFrame:GetRight() * XPerl_Player:GetEffectiveScale()
...
function XPerl_Player_SetGap(newGap)
	...
	if (type(newGap) == "number") then
		...
		local left = ((XPerl_Player_StatsFrame:GetRight() * XPerl_Player:GetEffectiveScale()) + newGap) / XPerl_Target:GetEffectiveScale()
