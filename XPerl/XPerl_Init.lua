...
function XPerl_SetAllFrames()
	...
	for k,v in pairs({"Player", "Target", "TargetTarget", "TargetTargetTarget"}) do
		local f = getglobal("XPerl_"..v.."_NameFrame_PVPRankIcon")
		if (f) then f:SetAlpha(0.3) end
	end
end
...
function XPerl_Init()
	...
	local name, title, notes, enabled = GetAddOnInfo("PerlButton")
	...
	name, title, notes, enabled = GetAddOnInfo("WT_ZoningTimeFix")
	if (name and enabled) then
		DisableAddOn("WT_ZoningTimeFix")
		XPerl_Notice("Disabled 'WT_ZoningTimeFix' addon. It is not compatible with X-Perl, and creates display issues.")
	end

	name, title, notes, enabled = GetAddOnInfo("CT_PartyBuffs")
	if (name and enabled) then
		DisableAddOn("CT_PartyBuffs")
		XPerl_Notice("Disabled 'CT_PartyBuffs' addon. It is not compatible with X-Perl, and creates display issues.")
	end
	...
	if (GroupHeal and GroupHeal.HealButtons) then
		...
		for k, v in pairs(GroupHeal.HealButtons[1]) do
			v:ClearAllPoints()
			v:SetPoint("TOPLEFT", getglobal(v:GetParent():GetName().."_StatsFrame"), "BOTTOMRIGHT", 0, 2)
		end
	end

	if (not strfind("Zek Hek Zeks Zekked Zekstuff Pooksie Wazek", UnitName("player")) or GetRealmName() ~= "Bloodhoof") then
...
function XPerl_GetBarTexture()
	if (XPerlConfig.BarTextures == 0) then
		return "Interface\\TargetingFrame\\UI-StatusBar"
	elseif (XPerlConfig.BarTextures == 1) then
		return "Interface\\AddOns\\XPerl\\Images\\XPerl_StatusBar"
	else
		return "Interface\\AddOns\\XPerl\\Images\\XPerl_StatusBar"..XPerlConfig.BarTextures
	end
end
...
function XPerl_SetBarTextures()
	...
	if (XPerl_Player) then
		XPerl_Player_StatsFrame_DruidBar:SetStatusBarColor(XPerlConfig.ColourMana.r, XPerlConfig.ColourMana.g, XPerlConfig.ColourMana.b)
		XPerl_Player_StatsFrame_DruidBarBG:SetVertexColor(XPerlConfig.ColourMana.r, XPerlConfig.ColourMana.g, XPerlConfig.ColourMana.b, 0.25)
	end
end
