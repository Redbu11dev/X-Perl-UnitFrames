...
local function DefaultVar(name, value)
	if (conf[name] == nil or (type(value) ~= type(conf[name]))) then
		conf[name] = value
	end
end
...
function XPerl_DefaultBarColours(reset)
	if (reset) then
		conf.ClassicHealthBar = nil
		conf.ColourHealthEmpty = nil
		conf.ColourHealthFull = nil
		conf.ColourMana = nil
		conf.ColourEnergy = nil
		conf.ColourRage = nil
		conf.ColourFocus = nil
	end
...
function XPerl_DefaultReactionColours(reset)
	if (reset) then
		conf.ColourReactionEnemy = nil
		conf.ColourReactionNeutral = nil
		conf.ColourReactionUnfriendly = nil
		conf.ColourReactionFriend = nil
		conf.ColourReactionNone = nil
	        conf.ColourTapped = nil
	end
...
function XPerl_Defaults()
	...
	if (not conf.BackColour.a) then
		conf.BackColour.a = 1
	end
	if (not conf.BorderColour.a) then
		conf.BorderColour.a = 1
	end
	...
	DefaultVar("ApplyToGuildList",		1)		-- 1.8.9
...
--	DefaultVar("HealerModeType",		1)		-- 1.8.3
	...
	local healerModeDefault = XPerlConfig.HealerMode
	if (not healerModeDefault) then
		healerModeDefault = 0
	end
	local healerModeDefaultType = XPerlConfig.HealerModeType
	if (not healerModeDefaultType) then
		healerModeDefaultType = 1
	end
	...
	DefaultVar("FullScreenStatusWarn",	20)
	DefaultVar("FullScreenStatusOK",	40)
	DefaultVar("HealerModePlayer",		healerModeDefault)	-- 1.9.1
	DefaultVar("HealerModePlayerType",	healerModeDefaultType)	-- 1.9.1
	...
	DefaultVar("HealerModeTarget",		healerModeDefault)	-- 1.9.1
	DefaultVar("HealerModeTargetType",	healerModeDefaultType)	-- 1.9.1
	DefaultVar("TargetChangeDefer",		1)		-- 1.9.5
	...
	DefaultVar("Scale_PartyPets",		conf.Scale_PartyFrame)
	DefaultVar("ShowPartyPetName",		1)
	DefaultVar("ShowPartyPetBuffs",		1)		-- 1.8.3
	DefaultVar("ShowPartyPetMana",		1)		-- 1.9.1
	DefaultVar("ShowPartyPetLevel",		1)		-- 1.9.1
	DefaultVar("ShowParty30YardSymbol",	1)
	DefaultVar("PartyBuffSize",		20)
	DefaultVar("HealerModeParty",		healerModeDefault)	-- 1.9.1
	DefaultVar("HealerModePartyType",	healerModeDefaultType)	-- 1.9.1
	...
	DefaultVar("HealerModeRaid",		healerModeDefault)	-- 1.9.1

	if (XPERL_RAIDGRP_PREFIX) then
		-- Move raid group positions from old config settings if user hasn't
		-- already placed them for new setup, otherwise delete old ones
		for i = 1,8 do
			if (conf.RaidPositions["Perl_Raid_Grp"..i]) then
				if (not conf.RaidPositions[XPERL_RAIDGRP_PREFIX..i]) then
					conf.RaidPositions[XPERL_RAIDGRP_PREFIX..i] = conf.RaidPositions["Perl_Raid_Grp"..i]
				end
				conf.RaidPositions["Perl_Raid_Grp"..i] = nil
			end
		end
	end

	XPerlConfig.HealerMode		= nil
	XPerlConfig.HealerModeType	= nil
end
...
function XPerl_Globals_OnEvent(event)
	if (event == "VARIABLES_LOADED") then
		...
		if ((not conf.ConfigVersion) or (conf.ConfigVersion ~= XPerl_VersionNumber)) then
			conf.ConfigVersion = XPerl_VersionNumber;	-- Set ConfigVersion to current
...
function XPerl_CancelFade(argFrame)
	if (argFrame.Fading == 1) then
		argFrame.Fading = 0
		argFrame:SetAlpha(conf.Transparency)
	end
end
...
function XPerl_ProcessFade(argFrame)
	if (argFrame.Fading == 1) then
		...
		if (argFrame.FadeTime >= 1) then
			...
			local newAlpha = conf.Transparency * (1 - argFrame.FadeTime)
