...
function XPerl_Target_SetDeBuffTooltip (target)
	...
	XPerl_ToolTip_AddDebuffDuration(target, nil, this:GetID(), XPerlConfig.TargetCurableDebuffs)
end
...
local function XPerl_Target_UpdateCombo()
	...
	if (XPerlConfig.UseCPMeter == 1) then
		...
		if (r) then
			XPerl_Target_CPMeter:SetStatusBarColor(r, g, b, 0.7)
		else
	...
	elseif (XPerlConfig.BlizzardCPMeter == 0) then
		...
		if (r) then
			XPerl_Target_CPFrameText:SetTextColor(r, g, b)
		else
			XPerl_Target_CPFrame:Hide()
		end
	end
...
local function XPerl_Target_DebuffUpdate()
	...
	if (GetComboPoints() == 0) then
		...
		if (XPerlConfig.UseCPMeter == 1) then
			...
			XPerl_Target_CPMeter:Show()
			if (r) then
				XPerl_Target_CPMeter:SetStatusBarColor(r, g, b, 0.4)
			else
				XPerl_Target_CPMeter:Hide()
			end
		else
...
local function XPerl_Target_UpdateManaType()
	if (XPerl_Target.Fading == 1) then
		return
	end

	XPerl_Target_SetManaType(XPerl_Target)
end

-- XPerl_Target_SetManaType
function XPerl_Target_SetManaType(frameFrame)
	local unit = frameFrame.unitid
	local frameManaBar = getglobal(frameFrame:GetName().."_StatsFrame_ManaBar")
	local frameManaBarText = getglobal(frameFrame:GetName().."_StatsFrame_ManaBarText")
	local frameStatsFrame = getglobal(frameFrame:GetName().."_StatsFrame")
	...
	if ((targetmanamax == 0) or (unit == "target" and XPerlConfig.ShowTargetMana==0) or (unit ~= "target" and XPerlConfig.ShowTargetTargetMana == 0)) then
		if (frameManaBar:IsShown()) then
			frameManaBar:Hide()
			frameStatsFrame:SetHeight(28 + (XPerlConfig.FatHealthBars * 2))
			XPerl_StatsFrameSetup(frameStatsFrame)
		end
		return
	end

	XPerl_SetManaBarType(unit, frameManaBar)
...
local function XPerl_Target_UpdateMana()
	...
	XPerl_Target_SetMana(XPerl_Target)
end

-- XPerl_Target_SetMana
function XPerl_Target_SetMana(thisFrame)
	local unit = thisFrame.unitid
	local frameManabar = getglobal(thisFrame:GetName().."_StatsFrame_ManaBar")
	local frameHealthBarPercent = getglobal(thisFrame:GetName().."_StatsFrame_HealthBarPercent")
	local frameManaBarPercent = getglobal(thisFrame:GetName().."_StatsFrame_ManaBarPercent")
	local frameManaBarText = getglobal(thisFrame:GetName().."_StatsFrame_ManaBarText")
...
local function XPerl_Target_UpdateHealth()
	...
	XPerl_Target_SetHealth(XPerl_Target, XPerlConfig.ShowTargetPercent)
...
function XPerl_Target_SetHealth(thisFrame, bPercent)
	local unit = thisFrame.unitid
	local thisid = thisFrame:GetName().."_StatsFrame_"
	local frameHealthBar = getglobal(thisid.."HealthBar")
	local frameHealthBarPercent = getglobal(thisid.."HealthBarPercent")
	local frameHealthBarText = getglobal(thisid.."HealthBarText")
	local frameManaBarPercent = getglobal(thisid.."ManaBarPercent")

	local targethealth, targethealthmax = UnitHealth(unit), UnitHealthMax(unit)

	if (targethealth > targethealthmax) then
	...
	if (thisFrame.feigning) then
		-- For coloured consistancy with other frames
		targethealth = 0
		targethealthmax = 1
	end
	...
	if (targethealthmax ~= 100) then
		if (XPerlConfig.HealerModeTarget == 1) then
			if (XPerlConfig.HealerModeTargetType == 1) then
	...
	if (bPercent == 1) then
		...
		elseif (thisFrame.feigning or UnitIsDead(unit)) then
			frameManaBarPercent:Hide()
			frameHealthBarPercent:SetText(XPERL_LOC_DEAD)
			if (thisFrame.feigning or XPerl_IsFeignDeath(unit)) then
				frameHealthBarText:SetText(XPERL_LOC_FEIGNDEATH)
			end
...
function XPerl_Target_Udpate_Range()
	if (not UnitIsVisible("target")) then
		if (not (UnitPlayerOrPetInParty("target") or UnitPlayerOrPetInRaid("target"))) then
			-- New bug in WoW 1.12.1, occasionally we don't receive PLAYER_TARGET_CHANGED when moving out of range of a unit
			-- So we check here now if they're still visible.
...
function XPerl_Target_UpdateDisplay()
	if (UnitExists("target")) then
		...
		if (XPerlConfig.TargetChangeDefer == 1) then
			XPerl_Target_NameFrame_ActivityStatus:Hide()
			XPerl_Target_PortraitFrame_Portrait:Hide()
			XPerl_Target_PortraitFrame_Portrait3D:Hide()
			XPerl_Target_NameFrame_LeaderIcon:Hide()
			...
			XPerl_Target_CPFrame:Hide()
			XPerl_Target_CPMeter:Hide()
			XPerl_Target_NameFrame_RaidIcon:Hide()
			this.deferring = true
			this.RangeUpdate = 0.5
		else
			XPerl_Target_UpdateLeader()
                	XPerl_Target_Update_Combat()
			XPerl_Target_UpdateCombo()
		...
		local buffOptionString = tostring(XPerl_Target_StatsFrame_ManaBar:IsVisible() or 0)..tostring(XPerl_Target_BossFrame:IsVisible() or 0)..tostring(XPerl_Target_CreatureType:IsVisible() or 0)
		if (XPerl_Target.buffOptionString ~= buffOptionString) then
			XPerl_Target.buffOptionString = buffOptionString
		...
		if (this.lastDeathMsg and this.lastTarget) then
			if (strfind (this.lastDeathMsg, this.lastTarget)) then
				...
				if (oldhp and strfind(oldhp, "/")) then
				...
				if (oldmana and strfind(oldmana, "/")) then
...
function XPerl_Target_OnUpdate()
	...
	if (this.Fading == 0) then
		this.RangeUpdate = this.RangeUpdate - arg1
		if (this.RangeUpdate <= 0) then
			this.RangeUpdate = 0.5
			XPerl_Target_Udpate_Range()

			if (this.deferring) then
				this.deferring = nil
				XPerl_Target_UpdateLeader()
                		XPerl_Target_Update_Combat()
				XPerl_Target_UpdateCombo()
...
-- Event Handler --
-------------------
function XPerl_Target_OnEvent()		-- TBC - function XPerl_Target_OnEvent(self, event)
...
function XPerl_Target_Events:UNIT_AURA()
	if (arg1 == "target") then
		...
		if (class == "HUNTER") then
			local f = XPerl_IsFeignDeath("target")
			if (f ~= this.feigning) then
				this.feigning = f
				XPerl_Target_UpdateHealth()
			end
		end
...
function XPerl_Target_Set_Bits()
	...
	XPerl_Target.buffOptionString = nil
